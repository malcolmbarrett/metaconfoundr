...
The metaconfoundr package is a toolkit for visualizing confounding control in a set of studies included in a meta-analysis. In this approach, a set of domain experts agree on the variables that are required to properly control for confounding for a scientific question. Then, for a given confounder, the studies are described as being adequately controlled, inadequately controlled, or controlled with some concerns (see the vignette on evaluating studies and setting up your data). metaconfoundr visualizes these relationships using heatmaps and traffic light plots. `metaconfoundr()` standardizes data for use in `mc_heatmap()` and `mc_trafficlight()`. Let's look at an example with an included data set, `ipi`. These data represent 14 analyses (retrospective cohorts and sibling-matched designs) to evaluate the association between short interpregnancy interval (<6 months versus 18-23 months) and risk of preterm birth (<37 weeks gestation) and the adequacy of confounder control. Using `metaconfoundr()` on `ipi` simply does some data wrangling to get it into a shape expected by the plotting functions:
...
The vignette on evaluating studies has more detail but in brief, the goal is to create a data frame where there are five columns and a row for each confounder and study. The columns are `construct`, the domain to which a confounder might belong (e.g. "Sociodemographics"), `variable`, the name of the variable (e.g. "age"), `is_confounder`, an indicator if the variable is a confounder, `study`, the name of the study (or another unique ID), and `control_quality`, an indicator of the level of control for a confounder. `control_quality` is one of: "adequate", "some concerns", or "inadequate". metaconfoundr attempts to detect the layout of your data automatically, but you have full control (see `?mc_detect_layout`). You can also specify the data in this format manually. 

Data that you provide `metaconfoundr()` can be in two basic formats: a long format and a wide format. The long format assumes that there are five columns, as above, and they simply need to be standardized. If there are more than five, `metaconfoundr()` assumes that, in addition to the other four columns, any additional columns represent studies (e.g. they are in wide format). It will automatically transform your wide data to the format expected by metaconfoundr plotting functions. `ipi` has a wide cousin, `ipi_wide`, which `metaconfoundr()` can prepare seamlessly:
...
mc_trafficlight(mc_ipi) + 
  ...
  scale_fill_cochrane() + 
  theme_mc() + 
  guides(x = guide_axis(n.dodge = 2)) # dodge axis text rather than rotate
```

It's also possible to sort plots by how well a confounder is controlled over all the studies included. See `?score_control` for more information on available algorithms to sort confounders by.
...
In addition to visualizing all possible confounders, metaconfoundr supports evaluating confounders at the domain level. For instance, if we feel `ipi` has three core areas of confounding, sociodemographics, socioeconomics, and reproductive history, we can specify what variables must be controlled for to account for the domain. For these 3, we'll say that controlling for maternal age, race/ethnicity, and marital status are sufficient to control for sociodemographics; socioecononmic status *or* insurance status *and* education are sufficient to control for socioeconomics; and prior pregnancy outcomes are sufficient to control for reproductive history. We can specify these rules using boolean logic that refers to confounders in the `variable` column of our data:
...
This creates a simpler visualization. You can also just visualize the overall control quality of a study by using the `domains = FALSE` argument in `summarize_control_quality()`.
...
sort_by_year <- function(.df) {
  .df %>% 
    arrange(year) %>% 
    mutate(
      study = paste0(lead_author, " ", year, " (", design, ")"),
...
Putting the plots side-by-side is easy with patchwork:
...
fp + theme(legend.position = "none") + 
  ...
  # make the FP twice as as wide as the TLP
...
Many studies control for variables that are not, in fact, confounders. While there are many cases where doing this will not effect results, there are several, such as controlling for a descendant of the exposure and outcome, that may increase bias. By default, metaconfoundr does not plot non-confounders. Instead, there is a set of tools for evaluating how many non-confounders a study controls for. The more controlled for, the higher the chance that one is inducing bias.
